    ...
    return (
        <LogedIn title="QuizPage" isAdmin>
            ...
            <header className="relative mb-10">
                <button onClick={()=>window.history.back()} className="absolute left-0 top-10 -translate-y-1/2">
                    <ArrowLeft size={48} color="gray" className="shadow rounded-2xl hover:bg-amber-300 transition-colors"/>
                </button>
                <h1 className="title title-big md:mt-0 mt-20 ">Quiz Details</h1>
                <Link href={route("admin.dashboard.quiz.edit",quiz.id)} className="absolute right-0 top-10 -translate-y-1/2">
