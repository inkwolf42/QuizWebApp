...
export default function ModifyPage({categories,quiz}:{categories:Array<CategoryIF>,quiz:QuizAdminIF}) {

    const [newChoice,setNewChoice] = useState("");

    const form = useForm({
        "category_id":quiz.category_id??-1,
        "question":quiz.question,
        "difficulty":quiz.difficulty,
        "has_multi_answer":quiz.has_multi_answer,
        "choices":quiz.choices as Array<ChoiceAdminCreateIF>
    })
    ...
    function onSubmit(e:any) {
        e.preventDefault()
        form.put(route("admin.dashboard.quiz.edit",quiz.id))
    }
    ...
    return (
        <LogedIn title="QuizPage" isAdmin>
            ...
            <form onSubmit={onSubmit} className="flex flex-col gap-5">
                ...
                <div>
                    <div className="flex flex-row bg-amber justify-start items-start ">
                        <div className="flex-1 flex flex-col">
                            <h1 className="font-bold text-2xl">Difficulty:</h1>
                            <p className="h-7 error">{form.errors.difficulty}</p>
                        </div>
                ...
                <h2 className="title text-3xl mt-16">Choices : </h2>
                <p className="h-7 error">{form.errors.choices}</p>
                <div className="flex flex-row gap-x-10">
                        <label htmlFor="choice-new" className=" font-bold text-xl">New Choice : </label>
                        <input
                            className="border-b-2 bg-gray-100 shadow focus:bg-gray-200 focus:border-amber-400 transition-all outline-none p-1 rounded-t-sm flex-1"
                        type="text" name="choice-new" id="choice-new" value={newChoice} onChange={e=>setNewChoice(e.target.value)}/>

                        <button className="button-default bg-green-200 hover:bg-green-300 transition-colors uppercase tracking-wider font-bold hover:text-white" type="button"
                            onClick={()=>{
                                if(form.data.choices.filter(choice=>choice.answer==newChoice).length!=0){
